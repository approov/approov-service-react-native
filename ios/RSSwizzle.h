...
/**
 ...
 one of its superclasses) has been already swizzled. Here the `RSSwizzleMode`
 and `key` parameters can help. See +[RSSwizzle
 ...
 @param RSSWReturnType The return type of the swizzled method wrapped in the
 RSSWReturnType macro.

 @param RSSWArguments The arguments of the swizzled method wrapped in the
 RSSWArguments macro.

 @param RSSWReplacement The code of the new implementation of the swizzled
 method wrapped in the RSSWReplacement macro.

 @param RSSwizzleMode The mode is used in combination with the key to indicate
 whether the swizzling should be done for the given class. You can pass 0 for
 RSSwizzleModeAlways.
 ...
 */
#define RSSwizzleInstanceMethod(classToSwizzle, selector, RSSWReturnType,      \
                                RSSWArguments, RSSWReplacement, RSSwizzleMode, \
                                key)                                           \
  _RSSwizzleInstanceMethod(classToSwizzle, selector, RSSWReturnType,           \
                           _RSSWWrapArg(RSSWArguments),                        \
                           _RSSWWrapArg(RSSWReplacement), RSSwizzleMode, key)
 ...
 */
#define RSSwizzleClassMethod(classToSwizzle, selector, RSSWReturnType,         \
                             RSSWArguments, RSSWReplacement)                   \
  _RSSwizzleClassMethod(classToSwizzle, selector, RSSWReturnType,              \
                        _RSSWWrapArg(RSSWArguments),                           \
...
/**
 RSSwizzleInfo is used in the new implementation block to get and call original
...
typedef NS_ENUM(NSUInteger, RSSwizzleMode) {
  ...
  /// RSSwizzle does not do swizzling if the same class has been swizzled
  /// earlier with the same key.
  RSSwizzleModeOncePerClass = 1,
  /// RSSwizzle does not do swizzling if the same class or one of its
  ...
  /// once per method call. If the order of swizzling is: first inherited class,
  /// second superclass, then both swizzlings will be done and the new
...
/**
 ...
 because of the fact that for safe and robust swizzling original implementation
 must be dynamically fetched at the time of calling and not at the time of
 swizzling, swizzling API is a little bit complicated.
...
#define _RSSwizzleInstanceMethod(classToSwizzle, selector, RSSWReturnType,     \
                                 RSSWArguments, RSSWReplacement,               \
                                 RSSwizzleMode, KEY)                           \
  [RSSwizzle swizzleInstanceMethod:selector                                    \
                           inClass:[classToSwizzle class]                      \
                     newImpFactory:^id(RSSwizzleInfo *swizzleInfo) {           \
                       RSSWReturnType (*originalImplementation_)(_RSSWDel3Arg( \
                           __unsafe_unretained id, SEL, RSSWArguments));       \
                       SEL selector_ = selector;                               \
                       return ^RSSWReturnType(_RSSWDel2Arg(                    \
                           __unsafe_unretained id self, RSSWArguments)) {      \
                         RSSWReplacement                                       \
                       };                                                      \
                     }                                                         \
                              mode:RSSwizzleMode                               \
...
#define _RSSwizzleClassMethod(classToSwizzle, selector, RSSWReturnType,        \
                              RSSWArguments, RSSWReplacement)                  \
...
#define _RSSWCallOriginal(arguments...)                                        \
  ((__typeof(originalImplementation_))                                         \
       [swizzleInfo getOriginalImplementation])(self, selector_, ##arguments)
