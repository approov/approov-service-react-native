...
* `options.sign` (boolean, optional): Defaults to `true`, so a policy mutator installed with this call applies HTTP Message Signing. This is the opt-in route; the out-of-box default mutator does not sign. Pass `{ sign: false }` to proceed per the mask and send the request unsigned (for apps that sign elsewhere or must not double-sign). Ignored for `MutatorPreset.DEFAULT`, which restores the unsigned default.
