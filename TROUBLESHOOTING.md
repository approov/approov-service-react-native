...
By default, network failures during an Approov token fetch will cause the mutator to abort the request before it leaves the device. This is surfaced to your Javascript `fetch()` or `axios` call as a standard network error (e.g., throwing a `TypeError: Network request failed` or an `IOException`). This is consistent with standard networking paradigms and should trigger your app's existing retry or offline fallback logic.

**Customizing Network Failure Behavior:**
By default, the `ApproovServiceMutator.DEFAULT` prevents requests from proceeding if a token cannot be fetched (`NO_APPROOV_SERVICE`, `MITM_DETECTED`, `POOR_NETWORK`, etc.). If you prefer to let these requests proceed (for example, allowing your backend to handle the rejection or gracefully degrade), you must perform a two-step configuration:
...
2. **Use `setUseApproovStatusIfNoToken(true)` (Optional):** Once your custom mutator allows the request to proceed, calling this JS function ensures the default interceptor writes the actual failure status (e.g., `MITM_DETECTED`, `POOR_NETWORK`) into the `Approov-Token` header. If you skip this step, the header is simply omitted or left empty.