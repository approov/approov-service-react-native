...
2. **`setUseApproovStatusIfNoToken` (on by default):** Whenever the mutator allows a request to proceed without a token (either by default behavior such as `NO_APPROOV_SERVICE`, or by your custom override), the interceptor writes the failure status (e.g., `NO_APPROOV_SERVICE`, `MITM_DETECTED`, `POOR_NETWORK`) into the `Approov-Token` header. This is **on by default** in the React Native service layer (note: this differs from the sibling Approov service layers, which default it off). Call `setUseApproovStatusIfNoToken(false)` if you instead want the header omitted/left empty.

## 6. Initialization failures
`ApproovService.initialize()` returns a promise that **rejects on failure**. Always `try/catch` it and do not make protected requests if it fails (see [USAGE.md → Handling Initialization Failures](USAGE.md#handling-initialization-failures)). Initialization parses the config locally and does **not** need network connectivity, so a valid config succeeds even offline. The rejection messages map to causes as follows:

| Rejection message (Android `initialize …` / iOS error detail) | Cause | Fix |
| :--- | :--- | :--- |
| `initialize IllegalArgument: …` (Android) / "…initialization failed…" (iOS) | The initial config string is malformed/invalid. Deterministic — fails every time, online or offline. | Use the exact config string from your onboarding email / `approov` CLI. |
| `initialize IllegalState: …` containing "already been initialized with a different configuration" | Another Approov service layer (or an earlier call) initialized the native SDK with a *different* config. The new config may be valid; the conflict is the issue. | Use one consistent config app-wide. Re-initializing with the **same** config is a safe no-op; to deliberately switch accounts pass `comment` starting `"reinit"`. |

> If you provision the config string remotely (e.g. Firebase Remote Config), remember the device may be offline at startup. A failed *remote fetch* is an app-level problem, not an Approov one: prefer bundling the (static) config in the app, cache a last-known-good value as a fallback, and **never pass `""` as a "couldn't load" placeholder** — `""` is the explicit bypass mode that disables Approov protection.
