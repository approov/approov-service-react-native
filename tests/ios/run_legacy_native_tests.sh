#!/bin/bash
#
# Legacy RN-specific native regression suite.
#
# This suite exercises the swizzled NSURLSession interception paths and
# ApproovMockURLProtocol infrastructure that the mini-sdk suite cannot reach.
# It covers the following 3.5.12 regressions:
#
#   - Mock response recursion guard (CHANGELOG: iOS Mock Response Recursion Fix)
#   - Mock completion handler preservation (CHANGELOG: iOS Mock Completion Handlers)
#   - dataTaskWithURL synthetic response path
#   - useApproovStatusIfNoToken via swizzled session path
#   - Initialization error edge cases (stub-based)
#
